package com.enss.ec.matrix;

import java.io.PrintStream;

import com.enss.ec.galois.Galois;
...
public class Matrix{
	...
	public void setContent(int[] content){
		if(cols*rows != content.length){
			throw new IllegalArgumentException(
					String.format("内容与矩阵的规格不匹配: cols=%d, rows=%d and content length: %d!",
	...
	public void rangeSet(int beginCol, int beginRow, Matrix other) {
		if(beginCol + other.cols > this.cols || beginRow + other.rows > this.rows){
			throw new IllegalArgumentException("目标矩阵的规格太小了!");
		}
	...
	protected Matrix rangeGet(int beginCol, int beginRow, int cols, int rows, Matrix dest){
		if(beginCol + cols > this.cols || beginRow + rows > this.rows){
			throw new IllegalArgumentException("源矩阵的规格太小了!");
		}
	...
	private Matrix convertToUpperTriangular(Matrix inverse, int w) {
		for (int col = 0; col < cols(); col++) {
			...
			if (this.get(col, col) == 0) {
				...
				if (row == rows()) {
					throw new RuntimeException("矩阵无法转换!");
				}
				this.swap_rows(col, row);
				inverse.swap_rows(col, row);
				
				if (this.get(col, col) == 0){
					throw new RuntimeException("矩阵在转换时出现了一些问题!");
				}
			}
