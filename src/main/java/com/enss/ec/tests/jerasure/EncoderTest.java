package com.enss.ec.tests.jerasure;
...
import com.enss.ec.jerasure.Buffer;
...
public class EncoderTest {
	...
	public void test_encode_small_file(){
		...
		try {
			k = 3; m = 1; w = 4;
			original = FileUtils.createFile("original", new byte[]{
					0x00, 0x26, 0x1e, 0x27, 
					0x52, (byte) 0xf6, 0x09, (byte) 0x85, 
					0x22, (byte) 0x97, 0x2e, 0x15, 
			});
			...
			k_files = FileUtils.collectFiles(original.getAbsolutePath(), "k", k);
			m_files = FileUtils.collectFiles(original.getAbsolutePath(), "m", m);
			...
			assertTrue(FileUtils.checkFileContent(k_files[0], new byte[]{0x00, 0x26, 0x1e, 0x27}));
			assertTrue(FileUtils.checkFileContent(k_files[1], new byte[]{0x52, (byte) 0xf6, 0x09, (byte) 0x85}));
			assertTrue(FileUtils.checkFileContent(k_files[2], new byte[]{0x22, (byte) 0x97, 0x2e, 0x15}));
			
			assertTrue(FileUtils.checkFileContent(m_files[0], new byte[]{0x70, 0x47, 0x39, (byte) 0xb7}));
	...
	public void test_encode_middle_file(){
		...
		try {
			...
			original = FileUtils.createFile("original", content);
			...
			assertTrue(FileUtils.checkFileContent(k_files[0], content_k1));
			assertTrue(FileUtils.checkFileContent(k_files[1], content_k2));
			
			assertTrue(FileUtils.checkFileContent(m_files[0], content_m1));
	...
	public void test_encode_with_smallest_overhead(){
		try {
			...
			File original = FileUtils.createFile("original", content);
			...
			Assert.assertEquals(len + CalcUtils.calcOverHead(original.length(), k, w), part_len);
	...
	public void test_file_extension_is_kept_as_it_was(){
		...
		try {
			...
			File original = FileUtils.createFile(name, "." + ext, content);
			
			Assert.assertEquals(ext, FileUtils.getExtension(original.getName()));
			...
			for(File part: k_files){
				...
				Assert.assertEquals(ext, FileUtils.getExtension(part.getName()));
			}
