...
public class FileUtils {
	...
	public static void deleteFiles(File[] files) {
		if(files == null){
			System.err.println("文件集合为null!");
			return;
		}
		for(File f: files){
			if(f != null && !f.delete()){
				System.err.println("无法删除: " + f.getAbsolutePath());
			}else if(f == null){
				System.err.println("文件为null!");
			}
		}
		...
		for (int i = 0; i < numParts; i++) {
			...
			try {
				...
				if (part.createNewFile()) {
					...
					throw new RuntimeException("文件块 " + part.getAbsolutePath()
							+ " 无法被创建!");
				}
			} catch (IOException e) {
				e.printStackTrace();
	...
	public static String getMD5Hash(File file) throws IOException, NoSuchAlgorithmException {
		...
		// an die L盲nge von 32bit anpassen
	...
	public static String getHash(File file, HashFunction hashFunc) throws IOException {
		...
		while (hashString.length() < hashLength) {
			// f眉hrende Nullen vorne hinzuf眉gen
			hashString = "0" + hashString;
		}
		return hashString;
	}
